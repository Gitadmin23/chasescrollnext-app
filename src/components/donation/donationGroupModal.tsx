    ...
    return (
        ...
        <LoadingAnimation loading={isLoading} >
            {data?.map((items, index) => {
                return (
                    <Flex role="button" flexDir={["column", "column", "row"]} onClick={() => router?.push("/dashboard/donation/group/" + items?.id)} key={index} w={"full"} rounded={"16px"} gap={"4"} borderWidth={"1px"} borderColor={borderColor} p={"4"} alignItems={"center"} >
                        <Flex w={"fit-content"} >
