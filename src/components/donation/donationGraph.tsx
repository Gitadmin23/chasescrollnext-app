...
export default function DonationGraph({ item, rounded, IsEvent, isPicked }: { item: IDonationList, rounded?: string, IsEvent?: boolean, isPicked?: boolean }) {
    ...
    return (
        <Flex w={"full"} borderWidth={(IsEvent || isPicked) ? "0px" : "1px"} borderTopWidth={"1px"} alignItems={"center"} borderColor={borderColor} rounded={rounded ? rounded : (IsEvent || isPicked) ? "0px" : "8px"} h={["fit-content", "fit-content", isPicked ? "fit-content" : "90px"]} px={rounded ? "20px" : (IsEvent || isPicked) ? "0px" : "8px"} pt={(IsEvent || isPicked) ? "2" : "0px"} justifyContent={"space-between"} >
            ...
            <Flex display={["flex", "flex", "none"]} >
                <CircularProgressBar isEvent={IsEvent || isPicked} size={(IsEvent || isPicked)  ? 40 : 67} strokeWidth={(IsEvent || isPicked)  ? 3 : 10} progress={((Number(item?.total) === 0) && (Number(item?.goal) === 0)) ? 0 : (Number(item?.total) / Number(item?.goal)) * 100 > 100 ? 100 : Number(((Number(item?.total) / Number(item?.goal)) * 100)?.toFixed(2))} />
            </Flex>
        </Flex>
    )
}
