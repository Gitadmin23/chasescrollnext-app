            ...
            <Grid templateColumns={["repeat(2, 1fr)", "repeat(2, 1fr)", "repeat(3, 1fr)", "repeat(3, 1fr)"]} gap={["2", "2", "6"]}  >
                {results?.map((item: IRental, index: number) => {
                    if (results?.length === index + 1) {
                        return (
                            <Flex ref={ref} key={index} w={"full"} h={"fit-content"} flexDir={"column"} bgColor={mainBackgroundColor} rounded={"16px"} p={["2", "2", "4"]} borderWidth={["1px", "1px", "1px"]} borderColor={borderColor} gap={["2", "2", "4"]} >
