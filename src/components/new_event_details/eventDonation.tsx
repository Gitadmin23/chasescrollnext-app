...
export default function EventDonation({ checkbox }: { checkbox?: boolean }) {

    const { borderColor, bodyTextColor } = useCustomTheme()
    ...
    return (
        <Flex role="button" flexDir={["row"]} w={"full"} rounded={"16px"} gap={["2", "2", "4"]} borderWidth={"1px"} borderColor={borderColor} px={["2", "2", "4"]} py={"4"} alignItems={"center"} >
            <Flex w={"fit-content"} >
                <Flex w={["80px", "80px", "150px"]} height={["80px", "80px", "150px"]} bgColor={"red"} rounded={"8px"} borderWidth={"1px"} borderColor={borderColor} >
            ...
            <Flex w={"full"} flexDir={"column"} gap={2} pr={"3"} >
                ...
                <DonationGraph item={items} IsEvent={true} />
            </Flex>
            {!checkbox && (
