...
function GetEventTicket(props: Props) {
    ...
    return (
        <>
            {!carousel && (
                <CustomButton bgColor={"brand.chasescrollBgBlue"} my={"auto"} onClick={clickHandler} disable={(!selectedTicket?.ticketType || selectedTicket?.ticketType || isBought) ? false : true} text={((isBought) ? "View" : isFree ? "Register" : "Buy") + " Ticket"} width={["full", "full"]} />
            )}
            {carousel && (
                <Box >
