        ...
        <>
            ...
            <ModalLayout open={open} close={closeHandler} closeIcon={false} >
                <Flex w={"full"} flexDir={"column"} gap={"4"} p={"5"} >
                    ...
                    <Button type="button" color={"white"} isLoading={editProfile?.isLoading} isDisabled={editProfile?.isLoading || (terms === true ? false : true) || !checkData?.username || userNameCheck?.includes("exists") || !checkData?.firstName || !checkData?.lastName} onClick={() => editProfile?.mutate(checkData)} mt={"4"} h={"50px"} w={"full"} borderWidth={"0.5px"} borderColor={"#233DF3"} bgColor={"#233DF3"} rounded={"32px"} gap={"3"} _hover={{ backgroundColor: "#233DF3" }} justifyContent={"center"} alignItems={"center"} >
