        ...
        <>
            {!request && (
                <>
                    ...
                    {(icon && name !== "Connect") ? (
                        <Flex disabled={loading === user_index ? true : false} _disabled={{cursor: "none"}} as={"button"} justifyContent={"center"} px={["1", "1", "3"]} alignItems={"center"} _hover={{ backgroundColor: "white" }} width={width ? width : "full"} rounded={"8px"} height={search ? "35px" : ["35px", "35px",  "43px"]} bg={name === "Pending" ? "#fff" : name === "Disconnect" ? "white" : "white"} borderColor={(name === "Pending" || name === "Disconnect") ? "" : "white"} borderWidth={(name === "Pending" || name === "Disconnect") ? "0px" : "0px"} color={name === "Pending" ? "#f78b26" : name === "Disconnect" ? "brand.chasescrollBlue" : "brand.chasescrollBlue"} fontSize={search ? "11px" :["xs", "xs",  "sm"]} fontWeight={["medium", "medium", "semibold"]}  >
