...
function CommunityImage(props: Props) {
    ...
    return (
        <Box roundedBottom={"64px"} roundedTopLeft={"64px"} borderColor={"#D0D4EB"} w={size} h={size} borderWidth={"2px"} >
            {data.data.imgSrc &&
                <Image style={{ borderBottomLeftRadius: "64px", borderBottomRightRadius: "64px", borderTopLeftRadius: "64px" }} objectFit="cover" alt={data.data.imgSrc || data.data.imgSrc} width={"full"} height={"full"} src={data.data.imgSrc?.includes("https://") ? data.data.imgSrc : IMAGE_URL + data.data.imgSrc} />
            }
            {(!data.data.imgSrc) && (
