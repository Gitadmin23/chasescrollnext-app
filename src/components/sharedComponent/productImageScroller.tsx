...
export default function ProductImageScroller({ images, userData, createdDate }: { images: Array<string>, userData?: IUser, createdDate?: string }) {
    ...
    return (
        <Flex cursor='pointer' w='full' h={["144px", "210px", "210px"]} bgColor={secondaryBackgroundColor} py={["3px", "3px", "2"]} borderRadius={'10px'} overflow={'hidden'} justifyContent={"center"} alignItems={"center"} position={'relative'} >
            <Flex position={"absolute"} left={"2"} top={"2"} bgColor={"#C4C4C499"} p={"1"} rounded={"full"} w={"fit-content"} alignItems={"center"} gap={2} >
...
{images?.length > 0 && (
            <Image rounded={"8px"} cursor='pointer' src={images[activeImageIndex].startsWith('https://') ? images[activeImageIndex] : (IMAGE_URL as string) + images[activeImageIndex]} alt="banner image" h='full' objectFit={"contain"} />
)}
        </Flex>
    )
}
