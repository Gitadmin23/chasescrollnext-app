    ...
    return (
        <Flex width={"full"} bgColor={"white"} roundedBottom={"6px"} padding={"6px"} justifyContent={"space-around"} >
            {tab_list?.map((item: {
                ...
                return( 
                    <Flex as={"button"} key={index} cursor={(item?.name === "Cash Out" || item?.name === "Fund Wallet") && type ? "not-allowed" : item?.name === "Fund Wallet" ?  "not-allowed" : "pointer"} opacity={(item?.name === "Cash Out" || item?.name === "Fund Wallet") && type ? "40%":  item?.name === "Fund Wallet" ? "40%" : ""} disabled={(item?.name === "Cash Out" || item?.name === "Fund Wallet") && type ? true : item?.name === "Fund Wallet" ? true : false} onClick={()=> clickHandler(index)} flexDirection={"column"} alignItems={"center"} fontSize={"12px"} justifyContent={"center"} py={"2"} rounded={"6px"} width={"full"} bgColor={tab === index ? "#12299C" : ""} color={tab === index ? type ? "brand.chasescrollYellow" : "white" : "#12299C"} borderColor={tab === index ? type ? "brand.chasescrollYellow" : "white" : "#12299C"} >
