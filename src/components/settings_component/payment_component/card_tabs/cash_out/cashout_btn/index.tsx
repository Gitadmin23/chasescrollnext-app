...
function CashoutBtn(props: Props) {
    ...
    const WithdrawFund = async (item?: any) => {
        ...
        try {
            ...
            toast({
                title: 'Error',
                description: error?.response?.data?.message?.includes("sufficient funds") ? "You do not have sufficient balance in this wallet" : error?.response?.data?.message?.includes("NGN") ? "You do not have sufficient balance in this wallet" : error?.response?.data?.message ,
                status: 'error',
                isClosable: true,
                duration: 5000,
                position: 'top-right',
