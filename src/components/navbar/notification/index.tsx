    ...
    return (
        <Box width={"auto"} position={"relative"} borderRadius={'10px'} >
             <Box position={'relative'}>
                <FiBell color={THEME.COLORS.chasescrollBlue} fontSize='30px' onClick={() => setActive(prev => !prev)} />
                <CustomText position={'absolute'} top='-10px' right='0'>3</CustomText>
             </Box>
            {active && (
                <Box width={"380px"} left={'-350px'} borderRadius={'10px'} height={'300px'} maxHeight={'500px'} zIndex={"20"} position={"absolute"} mt={"2"} >
                    <NotificationPage />
                </Box>
            )} 
            {active && (
