...
function NotificationCard({ notification }: {
  ...
  return (
    <HStack onClick={handleClick} alignItems={'center'} spacing={0} justifyContent={'space-between'} width='100%' height={'auto'} paddingY={'10px'} borderBottomWidth={'1px'} borderBottomColor={borderColor} paddingX='10px' bg={notification.status === "UNREAD" ? mainBackgroundColor:secondaryBackgroundColor}>
        <Box width='32px' height='32px' borderRadius={'36px 0px 36px 36px'} borderWidth={'1px'} borderColor={borderColor} overflow={'hidden'}>
        ...
        <VStack alignItems={'flex-start'} spacing={0} width='70%' flexWrap={'wrap'} paddingX='10px'>
            <CustomText fontSize={'14px'} fontFamily={'DM-Medium'} color={colorMode === 'light' ? 'brand.chasescrollButtonBlue':headerTextColor}>{notification.title}</CustomText>
