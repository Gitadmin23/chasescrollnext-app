    ...
    return (
        <Flex bg={mainBackgroundColor} w={["full", "full", "full", "full", "619px"]} pt={["4", "4", "4", "4", "8"]} px={["4", "4", "4", "4","8"]} >
            <Flex w={"full"} p={"4"} gap={"2"} rounded={"12px"} flexDir={"column"} style={{ boxShadow: "0px 2px 2px 0px #00000008" }} bgColor={mainBackgroundColor} h={"fit-content"} >
                <Flex pos={"relative"} w={"full"} bgColor={borderColor} gap={"1"} h={"fit-content"} alignItems={"start"} p={"2"} rounded={"12px"} >
                    ...
                    <Textarea value={post} onChange={(e) => setPost(e.target.value)} h={"45px"} bgColor={borderColor} w={"full"} borderWidth={"0px"} _hover={{ borderWidth: "0px", }} focusBorderColor='transparent' placeholder={`Share your thought ${!user?.username?.includes(".com") ? user?.username : ""}`} _placeholder={{ color: "#00000033" }} />
