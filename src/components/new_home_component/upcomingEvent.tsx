...
function UpcomingEvent(props: Props) {
    ...
    return (
        <Flex width={"full"} justifyContent={"center"} pt={"8"} h={"full"}  flexDirection={"column"} >
            <Text fontWeight={"semibold"} textAlign={"left"} fontSize={"20px"} mt={"15px"} pb={"10"}  ml={"8"}>{"Upcoming Event"}</Text>

            <Flex w={"full"} flexDirection={"column"} h={"full"} overflowY={"auto"} >
                ...
                <LoadingAnimation loading={isLoading} customLoader={
                    ...
                    <>
                        <Flex width={["full", "full", "full", "full", "437px"]} h={"fit-content"} px={"6"} flexDir={"column"} gap={"4"}>
                            {results?.map((event: any, i: number) => {
                                if (results.length === i + 1) {
                                    return (
                                        <Flex key={i} w={["full", "full", "full", "full", "full"]} ref={ref} >
                                            <ExploreEventCard landing={true} eventdashboard={true} date={true} page={true} event={event} />
                                        </Flex>
                                    )
                                } else {
                                    return (
                                        <Flex key={i + "last"} w={["full", "full", "full", "full", "full"]}  >
                                            <ExploreEventCard landing={true} eventdashboard={true} date={true} page={true} event={event} />
                                        </Flex>
                                    )
                                }
