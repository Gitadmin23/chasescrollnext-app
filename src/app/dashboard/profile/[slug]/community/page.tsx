                ...
                <LoadingAnimation  withimg={true} loading={isLoading} refeching={isRefetching} length={results?.length} >
                    {results?.map((community: any, i: number) => {
                        if (results?.length === i + 1) {
                            return (
                                <Box ref={ref} key={i} onClick={()=> router?.push(`/dashboard/community?activeID=${community?.id}`)} width={"full"} borderBottomWidth={"1px"} rounded={"3xl"} px={"4"} pb={"2"}>
                                    <CommunityCard profile={true} data={community} />
                                </Box>
                            )
                        } else {
                            return (
                                <Box key={i} onClick={()=> router?.push(`/dashboard/community?activeID=${community?.id}`)} width={"full"} borderBottomWidth={"1px"} rounded={"3xl"} px={"4"} pb={"2"}>
                                    <CommunityCard profile={true} data={community} />
                                </Box>
                            )
                        }
