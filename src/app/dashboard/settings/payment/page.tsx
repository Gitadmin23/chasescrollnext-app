        ...
        <Flex  px={"20px"} py={"30px"} overflowY={"auto"} width={"full"} flexDirection={"column"} > 
            ...
            <Flex mt={"6"} px={"6"} gap={"2"} flexDirection={"column"} >
                {PaymentPageList?.map((item: {
                    ...
                    return (
                        <Flex key={index} onClick={()=> router.push("/dashboard/settings/payment"+item?.route)} as={"button"} borderBottomWidth={"1px"} px={"4"} alignItems={"center"} borderColor={"gray.200"} color={"black"} fontSize={"15px"} py={"3"} gap={"1"}  >
                            {item?.name}
                        </Flex>
                    )
                })}
